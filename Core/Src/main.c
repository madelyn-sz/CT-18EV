...
#define BMS_TIMEOUT_MS  200          // 10 missed 0x600 frames at 50 Hz
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
	...
	if (RxHeader.StdId == 0x0B1) {
		...
		int16_t motor_speed_raw = (int16_t)(uint16_t)(RxData[3] << 8 | RxData[2]);
		...
		soc_kf_feed_bms((int16_t)(uint16_t)(RxData[7] << 8 | RxData[6]),
		                (uint16_t)(RxData[5] << 8 | RxData[4]),
		                RxData[2], RxData[1], HAL_GetTick());
	}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    			TxHeader.IDE = CAN_ID_STD;
    	  		...
    	  		/* SoC Kalman filter telemetry headers. These IDs are numerically
    	  		 * above the 0x0C0 command frame, so the inverter command always
    	  		 * wins CAN arbitration against them. */
...
//						}
    	  				...
    	  				/* TX failures are dropped, never fatal. 0x555/0x556 just took
    	  				 * two of the three mailboxes, so wait for one to clear. */
    	  				soc_kf_update(HAL_GetTick());
    	  				{
    	  					...
    	  					while (HAL_CAN_GetTxMailboxesFreeLevel(&hcan) == 0) {
    	  						uint32_t t = HAL_GetTick();
    	  						if (t < kf_wait || (t - kf_wait) > 3) {
    	  							break;
    	  						}
    	  					}

    	  					soc_kf_pack_state(kfData);
    	  					(void)HAL_CAN_AddTxMessage(&hcan, &socKfStateHeader, kfData, &TxMailbox);
    	  				}
