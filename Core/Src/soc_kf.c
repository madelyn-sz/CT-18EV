...
#ifdef SOC_KF_HOST
static inline uint32_t crit_enter(void)        { return 0u; }
...
static inline void crit_exit(uint32_t s) { __set_PRIMASK(s); }
#endif
...
static float lut(const float *tbl, float z, float *slope_out)
{
    ...
    int k = (int)x;
    if (k > SOC_KF_N_BP - 2)   k = SOC_KF_N_BP - 2;
...
{
    if (v <= soc_kf_ocv[0])                 return 0.0f;
    if (v >= soc_kf_ocv[SOC_KF_N_BP - 1])   return 1.0f;
...
void soc_kf_feed_bms(int16_t ibat_raw, uint16_t vbat_raw,
                     uint8_t btmp_raw, uint8_t soc_raw, uint32_t tick_ms)
{
...
void soc_kf_update(uint32_t tick_ms)
{
    ...
    uint8_t flags = 0;
    if (have_bms && (tick_ms - bms_tick) <= SOC_KF_STALE_MS) flags |= SOC_KF_FLAG_BMS_LIVE;

    /* Hold the estimate; never integrate silence. */
    if ((flags & SOC_KF_FLAG_BMS_LIVE) == 0u) {
        flags |= SOC_KF_FLAG_FROZEN;
        if (s.initialised) flags |= SOC_KF_FLAG_INIT;
        s.dbg.flags = flags;
        s.dbg.i_pack = i_pack;
        s.dbg.v_pack = v_pack;
    ...
    if (!v_ok) flags |= SOC_KF_FLAG_VBAD;
    ...
    if (s.init_method == 1) flags |= SOC_KF_FLAG_OCV_INIT;

    /* dt */
    float dt = (float)(tick_ms - s.last_update_tick) * 0.001f;
    if (dt <= 0.0f)  dt = 0.001f;
    if (dt > 0.5f)   dt = 0.5f;      /* guard against a scheduling hiccup */
    s.last_update_tick = tick_ms;
    ...
    if (s.p00 < SOC_KF_P_FLOOR) s.p00 = SOC_KF_P_FLOOR;
    if (s.p11 < SOC_KF_P_FLOOR) s.p11 = SOC_KF_P_FLOOR;

    if (s.z < 0.0f) { s.z = 0.0f; flags |= SOC_KF_FLAG_CLAMPED; }
    if (s.z > 1.0f) { s.z = 1.0f; flags |= SOC_KF_FLAG_CLAMPED; }
